...
```{r, echo=FALSE, fig.align='center', fig.height=4, fig.width=10}
fc_co2_2020 <- fabletools::forecast(fit_co2_full_trend, h=36)
...
> 'In Task 1a, we determined that the time series is not stationary. To remove the trend part of the time series, we performed the first differencing, and the new time series shows us that the trend is removed, but the seasonality is still there. The ACF shows oscillate correlations every 12 months, showing seasonality. So we need to perform a seasonal differencing to make the time series stationary.'   
...
> 'The Dickey-Fuller test rejected the null hypothesis, which is that the time series is not stationary. The time series plot also visually looks stationary now. Significant correlations at different lags on both ACF and PACF might suggest that the process is ARMA. The distribution is normal, which will help to construct an asymptotic confidence interval. Next, we will search for a model that minimizes AICc after applying first and seasonal differencing.'  
...
```{r prediction}
fc_100_years <- fabletools::forecast(model.bic, h=1236) %>%
  hilo() %>%
  unpack_hilo(c(`80%`, `95%`))
...
co2_present_season <- co2_present %>%
  model(stl = STL(average))
```

```{r, echo=FALSE, fig.align='center', fig.height=4, fig.width=10}
# Plot & check
s_plot <- components(co2_present_season) %>%
  as_tsibble() %>%
  autoplot(average, colour="gray") +
  geom_line(aes(y=trend), colour = "#D55E00") +
  labs(title = "CO2 Emissions with Seasonal Trend",
       x = "Date",
       y = TeX(r'($CO_2$ parts per million)')
  )
...
# Create data frame with season values 
df_season <- as_tsibble(data.frame(components(co2_present_season)))
...
far_plot <- fc_24_ns %>%
  autoplot(train_ns) +
  labs(
    title = "Forecasts of CO2 Level Up To 2022 Asing ARIMA",
    y = "CO2 Level", x = "Time"
  )
