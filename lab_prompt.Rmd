...
```{r}
# Getting the CO2 values since 1997
value <- co2_present %>% filter(year > 1997)
value <- value$average
...
data.frame(time_index = co2_present$time_index, actuals = co2_present$average, 
  ...
  mutate(lm_error = lm_forecast - actuals,
         arima_error = arima_forecast - actuals) -> forecast_perf_df
```
...
```{r linear time trend model residuals to present, echo=FALSE}
# Print residuals 
...
```{r quad time trend model residuals present, echo=FALSE}
# Print residuals 
...
```{r polynomial time trend with season model residuals present, echo=FALSE}
# Print results
...
```{r, arima present residuals, echo=FALSE}
# Print results
model.bic2 %>%
  gg_tsresiduals() +
...
```{r, residual box plots, echo=FALSE}
...
Finally, the p-value for all of the respective ljung_boxtest statistic is less than 0.05, except the ARIMA model.  Therefore, we can conclude that the residual values are independent for the ARIMA model only.
\newpage
...
```{r, test and train for 2 datasets}
...
```{r, check differencing for ns model, echo=FALSE}
...
```
```{r plot second difference for ns, echo=FALSE}
...
```{r, check differencing for s model, echo=FALSE}
# check the different values: 
s_diff <- train_s %>%
...
```{r polynomial based on underlying seasonal adjustments, echo=FALSE}
# Print results
fit_poly %>%
  gg_tsresiduals() +
